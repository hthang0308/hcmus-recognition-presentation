...
\begin{document}
	...
	\paragraph{Phonotactics characteristics - Đặc trưng ngữ âm} Mô tả cách sử dụng của người nói của các đơn vị ngữ âm và khả năng sử dụng có thể khác nhau giữa những người nói.
	...
	\qquad Để thực hiện phân tích phổ đáng tin cậy, các tín hiệu phải thể hiện các đặc tính tĩnh, điều này không dễ quan sát trong các tín hiệu giọng nói thay đổi liên tục. 
	...
	\begin{figure}[H]
		...
		\caption{Mel-Frequency based Cepstral Coefficients (MFCC) Diagram}
		\label{fig:writing-thesis}
	\end{figure}
	...
	Nguyên lý hoạt động của MFCC: Dữ liệu giọng nói thường được biểu diễn dưới dạng speech signal (tín hiệu giọng nói), hai chiều $(x, y)$ với $x$ là thời gian, $y$ là biên độ. Speech signal sẽ được biến đổi thành phổ âm (spectrum) bằng cách sử dụng phép biến đổi Fourier (Fourier Transformation, để thực hiện nhanh, người ta thường dùng Fast Fourier Transform Algorithm), kết quả là spectrum được biểu diễn lại ở hai chiều mới $(x', y')$, với $x'$ là tần số và $y'$ là cường độ. 
	...
	Chúng ta có một pipeline nguyên lý hoạt động của MFCC như sau: $\text{speech signal} \Rightarrow \text{spectrum} \Rightarrow \text{mel-freq filter} \Rightarrow \text{cepstral}$
	...
	\qquad Các đặc trưng ngữ điệu cơ sở như cao độ và năng lượng cũng có được ở mức frame. Năng lượng cửa sổ thu được rất dễ dàng thông qua định lý Parseval, ở dạng thời gian hoặc dạng phổ, và cao độ tức thời có thể được xác định bằng, ví dụ như, phương pháp tự động tương quan hoặc dựa trên phân rã cepstral, thường được làm mịn bằng một số lọc thời gian.
	...
	\begin{itemize}
		\item \textbf{Phương pháp dựa trên khuôn mẫu}: bao gồm một số chuỗi vector tương ứng với lời nói đăng ký và việc nhận dạng được thực hiện bằng cách so sánh lời nói xác minh với lời nói đăng ký. So sánh này được thực hiện bằng cách sử dụng Dynamic Time Warping (DTW) như một cách hiệu quả để cải thiện sai lệch thời gian giữa các cách phát âm khác nhau.
	...
	\begin{figure}[H]
		...
		\caption{Text-dependent speaker recognition with HMM speaker adaptation and HMM reestimation}
		\label{fig:writing-thesis}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption{The ATVS multilevel text-independent system}
		\label{fig:writing-thesis}
	\end{figure}
	...
	\begin{itemize}
		...
		\item Khả năng sử dụng cùng front-end (trích xuất đặc trưng) và back-end (so sánh vector) cho cả hệ thống i-vector và x-vector tạo điều kiện tích hợp hệ thống và cho phép so sánh trực tiếp hơn giữa hai phương pháp mô hình hóa.
	\end{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
	\end{itemize}
	\subsection{E.4 Đánh giá mô hình}
	Nhóm sử dụng môt số độ đo trong việc đánh giá mô hình như sau
	\begin{itemize}
		\item loss\_tr: là mất mát huấn luyện trung bình (tức là hàm cross-entropy) được tính ở mọi khung.
		\item err\_tr: là lỗi phân loại (đo ở mức khung) của dữ liệu huấn luyện. Lưu ý rằng chia tín hiệu giọng nói thành các phần 200ms với 10ms chồng lên nhau. Lỗi được tính trung bình cho tất cả các phần của tập dữ liệu huấn luyện.
		\item loss\_te là mất mát kiểm tra trung bình (tức là hàm cross-entropy) được tính ở mọi khung.
		\item err\_te: là phân loại sai (đo ở mức khung) của dữ liệu thử nghiệm.
		\item err\_te\_snt: là phân loại sai (đo ở mức câu) của dữ liệu kiểm tra.
	\end{itemize}
	...
	Ý nghĩa: 
	- Xác suất rằng một mẫu Positive được lấy ngẫu nhiên sẽ được xếp hạng cao hơn một mẫu Negative được lấy ngẫu nhiên. Biểu diễn theo công thức, ta có AUC = P(score(x+) > score(x-))
	...
	\subsection{E.5 Các kết quả}
	\subsubsection{E.5.1 Kết quả thực nghiệm trên tập TIMIT}
	\begin{figure}[H]
		\centering
	...
	\begin{figure}[H]
		...
		\caption{Đồ thị kết quả training TIMIT Dataset}
		\label{fig:writing-thesis}
	\end{figure}
	\begin{figure}[H]
		\centering
	...
	\begin{itemize}
		\item AP =  0.99
	\end{itemize}
	\subsubsection{E.5.2 Kết quả thực nghiệm trên tập Librispeech}
	\begin{figure}[H]
		\centering
	...
	\subsubsection{E.5.3 Kết quả thực nghiệm trên tập Son et al. Dataset}
	\begin{figure}[H]
		\centering
	...
	\begin{itemize}
		\item AP =  1.0
	\end{itemize}
	\subsubsection{E.5.4 Suy luận từ mô hình nhận dạng giọng nói tiếng Việt}
	Kiểm tra suy luận từ mô hình với giọng nói thu từ trình duyệt Google Colab
	\begin{figure}[H]
		\centering
		\includegraphics[width=.75\textwidth]{result/get_audio.png}
		\caption{Hàm thu âm giọng nói từ trình duyệt}
		\label{fig:writing-thesis}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=.75\textwidth]{result/get_sound_from_browser.png}
		\caption{Thu giọng nói từ trình duyệt}
		\label{fig:writing-thesis}
	\end{figure}
	Trực quan hình ảnh dạng tần số của âm thanh giọng nói
	\begin{figure}[H]
		\centering
		\includegraphics[width=.75\textwidth]{result/sound_hz.png}
		\caption{Trực quan hình ảnh dạng tần số của âm thanh giọng nói}
		\label{fig:writing-thesis}
	\end{figure}
	Tiến hành tải mô hình đã được huấn luyện, và đưa ra suy luận
	\begin{figure}[H]
		\centering
		\includegraphics[width=.75\textwidth]{result/test_inference_00.png}
		\caption{Suy luận từ mô hình}
		\label{fig:writing-thesis}
	\end{figure}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=.75\textwidth]{result/test_inference_01.png}
		\caption{Suy luận từ mô hình}
		\label{fig:writing-thesis}
	\end{figure}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Thử nghiệm dự đoán từ mô hình SincNet với tiếng Việt, bị overfitting do tập dữ liệu quá nhỏ.
		\end{itemize}
	\end{itemize}
	\nocite{*}
